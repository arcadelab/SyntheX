...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Training script for COVID-19 Lesion Segmentation Application.',
                                     formatter_class=argparse.ArgumentDefaultsHelpFormatter)

    # network model is first positional arg
    parser.add_argument('root_path', help='Path to the root folder, which contains train, valid, test data and checkpoint, runtime_figs, run_log_dir sub folders', type=str)
    ...
    parser.add_argument('--num-classes', help='The number of label classes to be identified', type=int, default=2)

    parser.add_argument('--batch-size', help='Number of images each minibatch', type=int, default=5)

    parser.add_argument('--ds-factor', help='Downsample factor', type=int, default=3)

    parser.add_argument('--unet-img-dim', help='Dimension to adjust input images to before inputting into U-Net', type=int, default=224)
    ...
    parser.add_argument('--max-num-epochs', help='Maximum number of epochs', type=int, default=10)
    ...
    valid_data_path = root_path + "/valid"

    if (not os.path.exists(train_data_path)) or (not os.path.exists(valid_data_path)):
        raise Exception("train or valid data folder not exists!")

    debug_fig_path = root_path + "/runtime_figs"
    if not os.path.exists(root_path + "/runtime_figs"):
        os.mkdir(root_path + "/runtime_figs")

    best_valid_filename = root_path + "/checkpoint/bestvalid_net.pt"

    train_loss_txt_path = root_path + "/checkpoint/train_loss.txt"

    valid_loss_txt_path = root_path + "/checkpoint/valid_loss.txt"

    if not os.path.exists(root_path + "/checkpoint"):
        os.mkdir(root_path + "/checkpoint")
    ...
    loss_fig_file = root_path + "/checkpoint/loss_fig.png"
    ...
    train_ds = COVIDRandomDataAugDataSet(train_data_path, train_data_num, proj_pad_dim=0, ds_factor=ds_factor, num_classes=num_classes, data_aug=data_aug, heavy_aug=heavy_aug)
    ...
    valid_ds = COVIDRandomDataAugDataSet(valid_data_path, valid_data_num, proj_pad_dim=0, ds_factor=ds_factor, num_classes=num_classes, data_aug=data_aug, heavy_aug=heavy_aug, valid=True)
    ...
    net = ViT_seg(config_vit, img_size=img_size, num_classes=config_vit.n_classes,
                  batch_norm=unet_batch_norm, padding=unet_padding, n_classes=num_classes, num_lands=0)
    ...
    while keep_training:
        ...
        def save_net(net_path):
            ...
            torch.save({ 'model-state-dict'     : net.state_dict(),},
                       tmp_name)
            shutil.move(tmp_name, net_path)

        checkpoint_filename = root_path + "/checkpoint/checkpoint_net_" + str(epoch).zfill(2) + ".pt"
