...
class ScaledupRandomDataAugDataSet(torch.utils.data.Dataset):
    ...
    def __getitem__(self, i):
        ...
        cur_lands_dim1 = np.expand_dims(f_lands['dim1'][:], axis=0)
        cur_lands_dim2 = np.expand_dims(f_lands['dim2'][:], axis=0)
        cur_lands_np   = np.concatenate((cur_lands_dim1, cur_lands_dim2), axis=0)
        cur_lands = torch.from_numpy(np.expand_dims(cur_lands_np, axis=0))
